...
let currentPageLink = document.querySelector('a[href = "contacts"]'), //инфор. какой линк активный   
...
/* searchInput.oninput = function(){
  const filterContact = contacts.filter((item)=> item.name.toLocaleLowerCase().startsWith(searchInput.value.toLocaleLowerCase()));
...
searchInput.addEventListener('keydown', (e) =>{
    ...
    const filterContact = contacts.filter((item)=> item.name.toLowerCase().startsWith(name));
    renderContacts(filterContact);
    }
} )
...
function renderAddContact(){
    ...
    form.onsubmit = formSubmitHandller;
}
...
function formSubmitHandller(event){
    event.preventDefault();
