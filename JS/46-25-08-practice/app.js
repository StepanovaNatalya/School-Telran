...
function renderUser(user){
  ...
  element.innerHTML = `
  <h2>${user.name}</h2>
  <h3>nick: ${user.username}</h3>
  <p>email: ${user.email}</p>
  `
  return element
}     

users.forEach(user => sect.append(renderUser(user)));

const userElements = sect.querySelectorAll('.user');
userElements.forEach(u => u.onclick = userClickHandler);

function userClickHandler(e){
   left.innerHTML = '';
   right.innerHTML = '';
   const userId = +e.currentTarget.id.split('_')[1];
   left.append(getAlbumsByUserId(userId));
}

const getAlbumsByUserId = (userId)=>{
  const element = document.createElement('div');
  const usersAlbums = albums.filter(el => el.userId === userId);
  if(!usersAlbums.length){
    element.innerHTML = 'No Albums';
    return element
  }
  usersAlbums.forEach(el => {
    element.append(renderAlbum(el))
  })
  return element
}
...
function renderAlbum(album){
  const element = document.createElement('div');
  element.id = 'album_' + album.id;
  element.innerHTML = `<h3>${album.title}</h3>`
  element.onclick = albumClickHandler;
  return element
}
...
function albumClickHandler(e){
  ...
  right.innerHTML = '';
  const albumId = +e.currentTarget.id.split('_')[1];
  right.append(getPhotoByAlbomId(albumId));
 
}

function getPhotoByAlbomId(id){
  const element = document.createElement('div');
  const photoByAlbumId = photos.filter(photo => photo.albumId === id);
  
  if(!photoByAlbumId.length){
    element.innerHTML = 'no photos';
    return element
  }
  photoByAlbumId.forEach(photo =>{
    element.append(renderPhoto(photo))
  })
...
function renderPhoto(photo){
  ...
  photoElement.id = 'photo_' + photo.id;
...
/* sect.onclick = (e) => {
  let target = e.target;
  console.log('Event target: ', target)     // 2й вариант клик на див через секцию
